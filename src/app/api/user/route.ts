...
export async function PATCH(request: Request) {
  ...
  if (hasInvalidStrategyText(targetTopics) || hasInvalidStrategyText(targetNiche)) {
    return NextResponse.json(
      {
        error:
          "Invalid onboarding content. Please enter real niche/topics, not system or route error messages.",
      },
      { status: 400 }
    );
  }
