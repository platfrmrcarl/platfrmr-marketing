...
function hasInvalidStrategyText(value: string) {
  const invalidPatterns = [
    /cannot have two parallel pages/i,
    /resolve to the same path/i,
    /please check\s+\/?\(protected\)/i,
    /\/\(protected\)\//i,
    /\/onboarding\b/i,
    /src\/app\//i,
  ];
...
export async function POST(req: Request) {
  try {
    ...
    if (hasInvalidStrategyText(userData.target_niche) || hasInvalidStrategyText(userData.target_topics)) {
      return NextResponse.json(
        {
          error:
            'Invalid target_niche/target_topics detected. Please update your profile strategy in the dashboard modal.',
        },
        { status: 400 }
      );
    }
