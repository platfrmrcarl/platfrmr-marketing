...
export async function GET(request: Request) {
  ...
  const callbackUrl = cookieStore.get("linkedin_callback_url")?.value || "/onboarding";
  ...
  try {
    ...
    const onboardingComplete = isNewUser
      ? false
      : (userSnap.data()?.onboarding_complete ?? false);
    ...
    const resolvedCallbackUrl =
      callbackUrl === "/onboarding" && onboardingComplete ? "/dashboard" : callbackUrl;
